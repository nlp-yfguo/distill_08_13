...
from utils.fmt.vocab.token import ldvocab,ld_freq_vcb,ld_weight_vcb
...
vcbt, nword = ld_freq_vcb("/home/yfguo/Data_Cache/wmt16/wmt16cache/rs/common.vcb") #a:1,b:2
print(vcbt)
_wei_list = []
weight = 0.0
for k,v in vcbt.items():
	if k == '<sos>' or k == "<eos>" or k == "<unk>" or k == "<pad>" :
		weight = 0.0
	else :
		weight = 1.0 / float(v)
	_wei_list.append(weight)
print("测试一下频率字典是否正常:",vcbt)
_wei_tensor = torch.tensor(_wei_list)
...
def train(_wei_tensor,td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		if mv_device:
			...
			_wei_tensor = _wei_tensor.to(mv_device)
		...
		with torch_autocast(enabled=_use_amp):
			...
			loss = lossf(output, ot, _wei_tensor)
			if multi_gpu:
				loss = loss.sum()
		...
		if nreport is not None:
			...
			if cur_b % nreport == 0:
				if report_eva:
					_leva, _eeva = eva(_wei_tensor,ed, nd, model, lossf, mv_device, multi_gpu, _use_amp)
...
def eva(_wei_tensor, ed, nd, model, lossf, mv_device, multi_gpu, use_amp=False):
	r = w = 0
	sum_loss = 0.0
	model.eval()
...
lrsch = LRScheduler(optimizer, lr_func=lambda a, b: (init_lr, b,))
...
minloss, minerr = eva(_wei_tensor, vd, nvalid, mymodel, lossf, cuda_device, multi_gpu, use_amp)
...
if fine_tune_m is None:
	...
	if cnt_states is not None:
		...
		if "training_list" in _remain_states:
			...
			shuffle(tl)
			_ctl = tl
		tminerr, done_tokens, cur_checkid, remain_steps, _ = train(_wei_tensor,td, _ctl, vd, nvalid, optimizer, lrsch, mymodel, lossf, cuda_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm, batch_report, save_every, chkpf, state_holder, statesf, num_checkpoint, cur_checkid, report_eva, remain_steps, False, False, scaler)
		_ctl = _remain_states = None
		vloss, vprec = eva(_wei_tensor, vd, nvalid, mymodel, lossf, cuda_device, multi_gpu, use_amp)
...
for i in range(1, maxrun + 1):
	shuffle(tl)
	free_cache(use_cuda)
	terr, done_tokens, cur_checkid, remain_steps, _Dws = train(_wei_tensor, td, tl, vd, nvalid, optimizer, lrsch, mymodel, lossf, cuda_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm, batch_report, save_every, chkpf, state_holder, statesf, num_checkpoint, cur_checkid, report_eva, remain_steps, dss_ws > 0, i >= start_chkp_save, scaler)
	vloss, vprec = eva(_wei_tensor,vd, nvalid, mymodel, lossf, cuda_device, multi_gpu, use_amp)
